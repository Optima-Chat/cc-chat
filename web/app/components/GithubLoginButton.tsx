...
export default function GithubLoginButton({ redirectTo }: GithubLoginButtonProps) {
  ...
  return (
    <button
      onClick={handleLogin}
      disabled={loading}
      className="px-4 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg hover:bg-gray-800 transition shadow-sm flex items-center gap-2 mx-auto disabled:opacity-50 disabled:cursor-not-allowed"
    >
      {loading ? (
        <>
